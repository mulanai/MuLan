...
class MultiViewUNetModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        t_emb = timestep_embedding(timestep, self.model_channels, repeat_only=False).to(sample.dtype)
        ...
        # imagedream variant
        if self.ip_dim > 0:
            sample[(num_frames - 1) :: num_frames, :, :, :] = ip_img # place at [4, 9]
            ip_emb = self.image_embed(ip)
            encoder_hidden_states = torch.cat((encoder_hidden_states, ip_emb), 1)

        h = sample
        for module in self.input_blocks:
            h = module(h, emb, encoder_hidden_states, num_frames=num_frames)
            hs.append(h)
        h = self.middle_block(h, emb, encoder_hidden_states, num_frames=num_frames)
